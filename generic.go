...
func (api API) filePathAndData(file interface{}) (string, interface{}, error) {
	...
	return filePath, fileContents, err
}
...
func (api API) upload(path string, file interface{}, input interface{}, output interface{}) error {
	...
	// nolint:errcheck
	backoff.Retry(func() error {
		filePath, fileContent, err := api.filePathAndData(file)
		if err != nil {
			errs = append(errs, err)
		}
		req := api.multiPart(gorequest.New().Post(prepareURL(path)).Type("multipart").SendFile(fileContent, filePath, "file", true).SendStruct(input))
